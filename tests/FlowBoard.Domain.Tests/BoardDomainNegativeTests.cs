...
public class BoardDomainNegativeTests
{
    ...
    private Board CreateBoardWithTwoColumnsAndCard(out ColumnId todoColumnId, out ColumnId doingColumnId, out CardId cardId)
    {
        ...
        var doingColumn = board.AddColumn("Doing", TestUserId).Value!;
        var card = board.AddCard(todoColumn.Id, "Card 1", null, TestUserId, _clock).Value!;
    ...
    {
        var board = Board.Create("Board", TestUserId, _clock).Value!;
        var firstAddResult = board.AddColumn("Todo", TestUserId);
        Assert.True(firstAddResult.IsSuccess);
        var duplicateAddResult = board.AddColumn("todo", TestUserId);
    ...
    {
        var board = Board.Create("Board", TestUserId, _clock).Value!;
        var todoColumn = board.AddColumn("Todo", TestUserId, 1).Value!; // WIP limit 1
        var firstCardResult = board.AddCard(todoColumn.Id, "Card 1", null, TestUserId, _clock);
        Assert.True(firstCardResult.IsSuccess);
        var secondCardResult = board.AddCard(todoColumn.Id, "Card 2", null, TestUserId, _clock);
    ...
    public void MoveCard_Fails_Card_Not_In_Source()
    {
        ...
        var todoColumn = board.AddColumn("Todo", TestUserId).Value!;
        var doingColumn = board.AddColumn("Doing", TestUserId).Value!;
        var card = board.AddCard(doingColumn.Id, "Card 1", null, TestUserId, _clock).Value!;
    ...
    {
        var board = Board.Create("Board", TestUserId, _clock).Value!;
        var sourceColumn = board.AddColumn("Todo", TestUserId).Value!;
        var targetColumn = board.AddColumn("Doing", TestUserId, 1).Value!; // WIP limit 1
        var existingCard = board.AddCard(targetColumn.Id, "Existing", null, TestUserId, _clock).Value!;
        var movingCard = board.AddCard(sourceColumn.Id, "Card 1", null, TestUserId, _clock).Value!;
    ...
    {
        var board = Board.Create("Board", TestUserId, _clock).Value!;
        var firstColumn = board.AddColumn("A", TestUserId).Value!;
        var secondColumn = board.AddColumn("B", TestUserId).Value!;
    ...
    {
        var board = Board.Create("Board", TestUserId, _clock).Value!;
        var result = board.Rename("", TestUserId);
        Assert.True(result.IsFailure);
    ...
    {
        var board = Board.Create("Board", TestUserId, _clock).Value!;
        var result = board.Rename("Board", TestUserId);
