...
public sealed class UpdateBoardHandler(IBoardRepository repository, IUserContext userContext) : IRequestHandler<UpdateBoardCommand, Result<BoardDto>>
{
    public async Task<Result<BoardDto>> Handle(UpdateBoardCommand request, CancellationToken cancellationToken)
    {
        ...
        var renameResult = board.Rename(request.Name, userContext.CurrentUserId);
